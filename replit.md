...
- **Context-Specific UPH Badge Conditional Formatting**: Implemented proper red-black-green color scale that compares UPH values only within same routing and work center context. Bottom 45% show red badges, median ±5% show black/gray badges, top 45% show green badges. Fixed issue where Assembly UPH for different product routings were incorrectly compared against each other. Added custom Badge variants (uphLow, uphMedium, uphHigh) to replace generic blue/red coloring with meaningful performance indicators
