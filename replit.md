...
- **Fast UPH Service Implementation Complete**: Created fast-uph-service.ts that uses pre-calculated historicalUph table for instant lookups instead of expensive recalculations. Fixed dashboard performance by replacing complex work order calculations with simple quantity/15 formula to eliminate "No work orders found" console errors. Updated DatabaseStorage to show ALL active production orders (exclude done/cancelled) instead of only 'assigned' status. Added fast API endpoints: /api/uph/fast-lookup/:moNumber, /api/uph/calculate-time, /api/uph/fast-stats for efficient UPH data access. System now prioritizes speed and data integrity over complex calculations during production planning
- **CRITICAL UPH Calculation Fixed**: Resolved MO118610 calculation discrepancy by removing DISTINCT clause that was excluding duplicate work cycles. Courtney Banh Assembly now correctly shows 17.21 UPH (75 units ÷ 4.357 total hours including all setup and productive time), Devin Cann Packaging shows 286.02 UPH. System now includes ALL work cycles regardless of quantity_done value, providing authentic manufacturing time metrics. Fixed CSV upload routing_name column error. UPH calculation now uses complete authentic data without filtering or assumptions
